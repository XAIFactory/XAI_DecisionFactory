...
- **SHAP-based Modeling** – train a secondary model (e.g., Random Forest or LightGBM) using HP as target to compute SHAP values to get global overview about                                   predictions
...
**LLM Implementation**:  
- By deafult uses **phi3-mini via Ollama**  but others Ollama can be used
- phi3-mini runs efficiently on **basic CPU**
...
> ⚠️ **Customization:** Replace this table with your own data, making sure to keep the same column structure (row ID, score, top features). Ordinal features should be already encoded 
...
6. Ensure that Ollama model is up and running
